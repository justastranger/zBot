...
SAXStream.prototype.on = function (ev, handler) {
  ...
  if (!me._parser["on"+ev] && streamWraps.indexOf(ev) !== -1) {
    me._parser["on"+ev] = function () {
      const args = arguments.length === 1 ? [arguments[0]]
...
function qname (name, attribute) {
  const i = name.indexOf(":"), qualName = i < 0 ? ["", name] : name.split(":");
    let prefix = qualName[0], local = qualName[1];
...
function attrib (parser) {
  ...
  if (parser.opt.xmlns) {
    const qn = qname(parser.attribName, true), prefix = qn.prefix, local = qn.local;
    ...
    if (prefix === "xmlns") {
      ...
      if (local === "xml" && parser.attribValue !== XML_NAMESPACE) {
        ...
        const tag = parser.tag, parent = parser.tags[parser.tags.length - 1] || parser;
        if (tag.ns === parent.ns) {
...
function openTag (parser, selfClosing) {
  if (parser.opt.xmlns) {
    ...
    let i = 0;
      const l = parser.attribList.length;
      for (; i < l; i ++) {
      const nv = parser.attribList[i];
      const name = nv[0], value = nv[1], qualName = qname(name, true), prefix = qualName.prefix, local = qualName.local;
