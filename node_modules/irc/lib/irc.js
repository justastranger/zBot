...
// BEGIN section added by justastranger

Client.prototype.kick = function(channel, args){
	var tmp = args.split(" "); // Split up args so that the name can be extracted
	var target = tmp[0]; // The name should be the first item in the array
	tmp.shift(); // Remove the name from the array
	var text = tmp.join(" "); // Convert it back to a string to use as the kick reason
	this.send("KICK", channel, target, text);
};

Client.prototype.ban = function(channel, mask){
	this.send("MODE", channel, "+b", mask);
};

Client.prototype.unban = function(channel, mask){
	this.send("MODE", channel, "-b", mask);
};

Client.prototype.quiet = function(channel, mask){
	this.send("MODE", channel, "+q", mask);
};

Client.prototype.unquiet = function(channel, mask){
	this.send("MODE", channel, "-q", mask);
};

Client.prototype.op = function(channel, mask){
	this.send("MODE", channel, "+o", mask);
};

Client.prototype.deop = function(channel, mask){
	this.send("MODE", channel, "-o", mask);
};

Client.prototype.voice = function(channel, mask){
	this.send("MODE", channel, "+v", mask);
};

Client.prototype.devoice = function(channel, mask){
	this.send("MODE", channel, "-v", mask);
};
...
Client.prototype.who = function(nick, callback) {
	if (typeof callback === 'function') {
		var callbackWrapper = function(info) {
